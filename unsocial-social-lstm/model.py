...
class Model():
    ...
    def __init__(self, args, infer=False):
        ...
        cell = rnn.BasicLSTMCell(args.rnn_size, state_is_tuple=False)
        ...
        def tf_2d_normal(x, y, mux, muy, sx, sy, rho):
            ...
            z = tf.square(tf.div(normx, sx)) + tf.square(tf.div(normy, sy)) - 2*tf.div(rho * (normx * normy), sxsy)
            negRho = 1 - tf.square(rho)
            # Numerator
            result = tf.exp(tf.div(-z, 2*negRho))
            # Normalization constant
            ## denom = 2 * np.pi * tf.mul(sxsy, tf.sqrt(negRho))
            denom = 2 * np.pi * (sxsy * tf.sqrt(negRho))
