...
def main():
    ...
    parser.add_argument('--dropout_prob', type=float, default=0.2,
                        help='fraction of nodes to drop during training')
...
def train(args):
    ...
    with open(os.path.join('fish/save', 'config.pkl'), 'wb') as f:
        pickle.dump(args, f)
        ...
        for e in range(args.num_epochs):
            ...
            for b in range(data_loader.num_batches):
                ...
                feed = {model.input_data: x,
                        model.target_data: y,
                        model.dropout: args.dropout_prob,
                ...
                if (e * data_loader.num_batches + b) % args.save_every == 0 and ((e * data_loader.num_batches + b) > 0):
                    ...
                    checkpoint_path = os.path.join('fish/save', 'model.ckpt')
                    saver.save(sess, checkpoint_path, global_step=e * data_loader.num_batches + b)
                    print("model saved to {}".format(checkpoint_path))

        # finished: save one additional time
        checkpoint_path = os.path.join('fish/save', 'model.ckpt')
        saver.save(sess, checkpoint_path, global_step=args.num_epochs * data_loader.num_batches + data_loader.num_batches)
        print("Finished training! Model saved to {}".format(checkpoint_path))
