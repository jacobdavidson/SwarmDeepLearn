...
Adapted from Anirudh Vemula's code
...
class Model():
    ...
    def __init__(self, args, infer=False):
        ...
        for _ in range(args.num_layers):
            ...
            cell = rnn.DropoutWrapper(cell, output_keep_prob=1.0 - self.dropout)
            cells.append(cell)
...
#        outputs, last_state = tf.nn.dynamic_rnn(cell, embedded_inputs, initial_state=self.initial_state, scope="rnnlm")
    ...
    def sample(self, sess, traj, true_traj, bias=0.0, num=10, dropout=0.0):
        '''
        ...
        bias: Weight of bias to be applied to more likely locations
        num: Number of time-steps into the future to be predicted
        dropout: Fraction of nodes to drop out
        '''
        ...
        for t in range(num):
            ...
            # Adjust standard deviations to incorporate bias
            o_sx = np.exp(np.log(o_sx) - bias)
            o_sy = np.exp(np.log(o_sy) - bias)
